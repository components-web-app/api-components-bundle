...
| `silverback:api-components:scan-orphaned` (alias `clean-orphaned`) | The scheduled-run path (#348): scans synchronously through `ScanOrphanedResourcesHandler::scan()`, stores the report, prints counts per kind (IRIs with `-v`), exits 0. |
...
- `OrphanedResourceHelper` has no check-and-remove methods any more; do not reintroduce a second deletion path. Keep the report behind `OrphanedResourceReportStore` so its storage can be swapped (#348).

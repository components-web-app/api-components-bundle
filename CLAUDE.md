...
### API Platform 5 / Symfony 8.x compatibility notes
...
- **API Platform 5 only (#287).** The constraint is `api-platform/core: ^5.0`. Dual `^4.4 || ^5.0` support was built and passed on both versions with no version branches, then dropped, because no bundle release is declared stable and supporting 4.4 was only costing CI time. What 5.0 changed for the bundle:
  ...
  - **Denormalization errors use API Platform 5's shape.** A denormalization error on a property with constraints is a 422 with violations, where 4.x gave a 400. 5.0 also adds `UniqueConstraintViolationException` → 422 to its default map, which #293's compiler pass picks up.

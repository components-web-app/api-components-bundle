...
### API Platform 4.4/5 / Symfony 8.x compatibility notes
...
- **API Platform `^4.4 || ^5.0` are both supported (#287), with no version branches in code.** The Behat CI job has an `api-platform` dimension (`^4.4`, `^5.0`). The coverage run uses 5.0, and the lowest-dependencies job installs 4.4.0. Support was briefly narrowed to `^5.0` (#296) and restored, because 5.0.0 was only a week old with no patch release. The whole suite passes on both versions. What 5.0 changed for the bundle:
  - **`/me` kept its `@type`.** `UserResourceMetadataCollectionFactory` used to give the `/me` operation `shortName: '__api_me'`. On 4.4 that already leaked into `@context` (`/contexts/__api_me`), while `@type` stayed `User`; 5.0 also emits it as `@type`. The operation now keeps the resource's own short name, so both versions return `@type: User` and `@context: /contexts/User`, which `features/user/me.feature` pins. The `@context` correction is a change on 4.4 too.
  - **Relations stay IRIs.** API Platform #8283 can set `readableLink` for a relation into a JOINED hierarchy when a subclass shares a serialization group. The schema files already require `ComponentPosition.component` and `Route.pageData` (both JOINED targets) to be IRI strings, and they pass on 5.0. If a future change makes a relation embed, those schema checks are what should catch it.
  - **Denormalization errors follow the installed version, deliberately not normalised.** On 5.0 a denormalization error on a property with constraints is a 422 with violations, where 4.4 gives a 400. 5.0 also adds `UniqueConstraintViolationException` → 422 to its default map, which #293's compiler pass picks up. A test that asserts a denormalization status must hold on both versions: the test app's `NotNormalizableValueException` → 422 mapping is why `exception_to_status.feature` does.
  - **Nothing in `src/` may depend on a 5-only API** while 4.4 is supported. The query-parameter filters #289 uses (`FreeTextQueryFilter`, `OrFilter`, `PartialSearchFilter`, `ExactFilter`, `SortFilter`) all exist in 4.4.
- **API Platform's default `exception_to_status` entries are appended by the bundle (#293).** API Platform declares its map (serializer exceptions and its `InvalidArgumentException` → 400, `OptimisticLockException` → 409) as the config node's `defaultValue`, and a `defaultValue` only applies when nothing supplies the node. The bundle prepends its own four entries, so the defaults were silently lost in every application and those exceptions were 500s. `ApiPlatformCompilerPass` now appends them to the `api_platform.exception_to_status` parameter, read from API Platform's own configuration tree so a new default in a later version (5.0 adds `UniqueConstraintViolationException` → 422) is picked up without a change here. **They are appended, never prepended:** the first matching class wins, and a prepended `SerializerExceptionInterface → 400` would override an application's more specific serializer mapping (the test app maps `NotNormalizableValueException` → 422). An application's own status for a default class is kept. Tests: `features/main/exception_to_status.feature`, `tests/DependencyInjection/CompilerPass/ApiPlatformCompilerPassTest.php`.
...
- **PHPStan runs in CI at level 5 against a baseline** (`phpstan.neon.dist`, `phpstan-baseline.neon`). It replaced Scrutinizer, whose PHP mirror has no 8.5 build. The baseline records the 102 findings that existed when it was introduced, so only new errors fail the job. More than a quarter are `method.notFound` and are worth working through; fixing one means regenerating the baseline with `vendor/bin/phpstan analyse --generate-baseline phpstan-baseline.neon`. An entry that no longer matches also fails the job, so a fix that removes a finding must remove its baseline entry too. `src/Resources/skeleton` is excluded: its `.tpl.php` files are code-generation templates, not PHP. The Doctrine, Symfony and PHPUnit extensions load through `phpstan/extension-installer`.
...
### #289 — The bundle's filters are API Platform `QueryParameter`s (module: cwa-nuxt-module#328, template: components-web-app#89) ✓ **DONE**

API Platform deprecated `#[ApiFilter]`, `SearchFilter`, `OrderFilter` and `AbstractFilter`. Route, Layout and Page now declare `parameters:` instead, and `src/` has no `#[ApiFilter]` left.

| Parameter | Filter | On |
|---|---|---|
| `search` | `FreeTextQueryFilter(new OrFilter(new PartialSearchFilter()))` | Layout: `reference`, `uiComponent`. Page: `title`, `reference`, `uiComponent`. Route: `path` |
| `order[:property]` | `SortFilter` | Layout, Page: `createdAt`, `reference`. Route: `createdAt`, `path` |
| `isTemplate` | `ExactFilter` + `BooleanQueryValue` | Page |

- **Search is one parameter across fields.** `?reference=x&uiComponent=x` became `?search=x`. The per-field parameters are now ignored, not refused, which is what lets the module send both during the transition. Sorting URLs are unchanged, but an invalid direction is now a 422 instead of being ignored.
- **`FreeTextQueryFilter` must always wrap `OrFilter`.** A bare `OrFilter` ORs against the whole WHERE clause and discards the publication and draft predicates. That is the #237 bug. `features/main/or_search_filter.feature` pins that `?search=` still excludes scheduled, draft and ancestor-gated routes.
...
### #311 — A failed HTTP cache purge no longer turns a saved write into a 500 ✓ **DONE**

The twin of #283 for the cache. `HttpCachePurger::propagate()` runs from `PropagateUpdatesListener::postFlush()`, after the commit. With the cache unreachable or answering an error status, the write got a 500. Every propagator after the purger was skipped too, which includes the Mercure publisher, because `services_doctrine_orm_http_cache_purger.php` is loaded first.

**Where it is thrown, verified rather than assumed.** API Platform's `SurrogateKeysPurger::purge()` (the base of both `SouinPurger` and `VarnishXKeyPurger`) calls `$client->request(...)` as a bare statement and discards the response. Its refcount drops to zero at the end of that statement, so Symfony HttpClient's response destructor runs **there, inside `purge()`**, and throws: `ServerException`/`ClientException` for a 5xx/4xx, `TransportException` when the host cannot be reached. It is never deferred to the end of the request or to GC. This was reproduced with `MockHttpClient` behind a real `ScopingHttpClient`, with and without `TraceableHttpClient`, for both purgers, and with a real `CurlHttpClient` against a closed port. A `try` around `purge()` therefore catches it. The first failure also abandons the rest of `purge()`'s loop, so any later header chunks and any other invalidation URLs are not sent for that flush.

**Two paths, two behaviours.** The private `send()` wraps a failure in `HttpCachePurgeFailedException`, carrying the tags and the original exception as `previous`. Then:
- **After a write, it is logged and swallowed.** `propagate()` logs at **error** level, with the tags and the original exception in the context, and the write gets its normal response. The other propagators still run. Unlike Mercure it must not be quiet, because the tags stay cached for up to `s-maxage` (a year in the template). `reset()` is now in a `finally`, so a failed purge's tags are not resent with the next flush and nothing survives under worker mode.
- **An explicit purge fails loudly.** `purgeRenderedHtml()` lets the exception out. `silverback:api-components:purge-rendered-html` prints the reason and exits **1**, as `purge-http-cache` does for a failed flush. `POST /_/rendered_html/purge` returns **502**.

**Why 502.** The API is acting as a gateway to the cache, and the cache failed or could not be reached. 503 would claim the API itself is unavailable, and a 500 says nothing. In production API Platform replaces the `detail` of any 5xx with "Internal Server Error", so the status code is the only signal the admin button gets, and it should be specific. **`HttpCacheFlushFailedException` (#290) is mapped to 502 in the same change.** Before, it was not mapped, so a failed flush on `POST /_/http_cache/purge` was a bare 500. Both mappings are in the bundle's `exception_to_status` prepend.

**What is caught:** `Symfony\Contracts\HttpClient\Exception\ExceptionInterface` (every status and transport error above) and `ApiPlatform\Metadata\Exception\RuntimeException`. The second is what `SurrogateKeysPurger` throws when one tag is longer than `max_header_length`. That depends on the data written rather than on the deployment, and the write has already happened. Not `\Throwable`, for #283's reason: a programming error still propagates. It is pinned for both paths.

**Wiring:** the logger is the purger's last constructor argument, `new Reference('logger', NULL_ON_INVALID_REFERENCE)`.

**Tests.** `tests/HttpCache/HttpCachePurgerTest.php` runs the real `VarnishXKeyPurger` and `SouinPurger` over `MockHttpClient` for a 500, a transport error and a tag that is too long. It also covers the log's level, tags and exception, no logger, no carry-over of a failed purge's tags, a `LogicException` not swallowed on either path, `purgeRenderedHtml()` throwing `HttpCachePurgeFailedException` for a status and for a transport error, and the logger reference in the service definition. Further tests: `tests/Command/PurgeRenderedHtmlCommandTest.php` (exit 1 with the reason), `tests/DataProcessor/StateProcessor/RenderedHtmlPurgeStateProcessorTest.php`, and `tests/DependencyInjection/SilverbackApiComponentsExtensionTest.php` (both 502 mappings). Behat: `features/main/http_cache_purge_failure.feature` covers a `Layout` POST with the cache unreachable, which returns 201, saves the resource, logs the failure with its IRI in the tags and still publishes to Mercure. It also covers a following scenario proving the mode resets, and an admin `POST /_/rendered_html/purge` returning 502. `MockClientCallback` gained a static unreachable switch that fails only `PURGE` requests, set by `Given the HTTP cache is unreachable` and cleared in `ProfilerContext`'s hooks, the same shape as `HubStub`. New steps: `the cache purge failure for the resource :name should have been logged`, `no cache purge failure should have been logged`, `a Mercure update should have been published for the resource :name`. All were watched failing first. The Behat write and the endpoint both got a 500, and with the old purger the same write published **zero** Mercure updates.

References: `src/HttpCache/HttpCachePurger.php`, `src/Exception/HttpCachePurgeFailedException.php`, `src/Command/PurgeRenderedHtmlCommand.php`, `src/DependencyInjection/SilverbackApiComponentsExtension.php`, `src/Resources/config/services_doctrine_orm_http_cache_purger.php`, `tests/Functional/MockClientCallback.php`, `features/bootstrap/ProfilerContext.php`.

---

### #286 — References to classes removed from API Platform, Lexik, Doctrine and Symfony ✓ **DONE**

Found by a PHPStan level-0 run during the dependency update (#285). A missing class in a `use` statement costs nothing until the line runs, and then it is a 500 with `Class "…" not found`.

**`OrSearchFilter` was the real bug.** It threw `ApiPlatform\Exception\InvalidArgumentException`, which API Platform 4 no longer has; the class is `ApiPlatform\Metadata\Exception\InvalidArgumentException`. An anonymous `GET /dummy_or_search_filterables?rank=not-an-integer` was therefore a 500. It now does what API Platform's own `SearchFilter` does: logs `notice` `Invalid filter ignored` with the exception in the context and drops that field's clause. The rest of the request's filter clauses still apply. The same fix covers an association whose identifier is an integer. Only integer-typed fields can hold an invalid value (`SearchFilterTrait::hasValidValues()`), so the bundle's own filters, which are all on strings, were never affected; an application filtering an integer column was. An unknown strategy is a misconfiguration and still throws, now the intended `InvalidArgumentException`.

> **The bundle's `exception_to_status` prepend replaces API Platform's defaults.** `ApiPlatform` declares its map with `defaultValue()`, so any configured value, prepended or not, wipes `SerializerExceptionInterface → 400`, `InvalidArgumentException → 400` and `OptimisticLockException → 409` in every application using this bundle. The unknown strategy is therefore a 500, not a 400. Not fixed here; it needs its own issue.

**Dead references removed, no behaviour change:** the `Legacy*` `ApiPlatform\Exception\*` aliases in the multi-catches of `MercureResourcePublisher`, `MercureAuthorization`, `OpenApiFactory` and `UuidUriVariableTransformer`; the `Address::fromString()` fallback in `MessageEventListener` and `AbstractUserEmailFactory` (removed in symfony/mime 6, and `method_exists(Address::class, 'create')` is always true); `SqlitePlatform` → `SQLitePlatform` in `SqlLiteForeignKeyEnabler`; `Doctrine\Common\Proxy\Proxy` → `Doctrine\Persistence\Proxy` in `PageDataProvider`, matching `ComponentPositionNormalizer` and `PropagateUpdatesListener`. doctrine-bundle 3 always enables native lazy objects, so an ORM proxy is an instance of the real class and that branch cannot run either way. In the test harness, `UploadsContext::getUploadableResourceByName()` read an undefined `$minkContext` on its not-found path and now throws a plain exception.

**`JWTManager::decode()` no longer refreshes an expired token.** Its fallback built a `PreAuthenticationJWTUserToken`, which Lexik 3 removed. Nothing calls `decode(TokenInterface)`: in Lexik 3.2 `JWTAuthenticator::authenticate()` and `BlockJWTListener` both call `parse(string)`, and no bundle code calls `decode()`. The expired-token refresh lives in `parse()`, which the authenticator does reach, and is pinned by "Expired JWT tokens should be refreshed" in `features/user/security.feature`. `decode()` now delegates and lets `JWTDecodeFailureException` propagate, as the interface documents; before, a direct caller holding an expired token got a class-not-found error.

Tests: `tests/Mercure/MercureAuthorizationTest.php`, `tests/Mercure/MercureResourcePublisherTest.php`, `tests/OpenApi/OpenApiFactoryTest.php` and `JWTManagerTest::create()` cover the catch branches whose aliases were removed, which no test had reached. `tests/Filter/OrSearchFilterTest.php` (an invalid value for a field and for an association identifier is logged and ignored while the other clause applies, a valid integer is applied, an unknown strategy throws the Metadata exception); `tests/Security/JWTManagerTest.php`; `features/main/or_search_filter.feature` (200 with the clause ignored and the notice logged; the ignored clause leaves the other one applied). `DummyOrSearchFilterable` gained a nullable integer `rank` for this, and `ProfilerContext` gained `an ignored filter value for the field :field should have been logged`, read from `app.monolog.test_handler`. All were watched failing first: the unit tests on `Class "ApiPlatform\Exception\InvalidArgumentException" not found`, the Behat scenarios on a 500, the `decode()` test on a refresh attempt.

**Left alone:** `JetBrains\PhpStorm\Pure` and `ArrayShape` attributes in `AttributeReader`, `UploadableResourceMetadataCollectionFactory` and `ApiEventListenerTrait`. The classes are not installed, but PHP never instantiates an attribute unless something calls `newInstance()` on it, so they are IDE hints only.

...
| `POST /_/orphaned_files/scan`, `GET /_/orphaned_files`, `POST /_/orphaned_files/delete` | Orphaned and missing stored files (`ROLE_ADMIN`). See **Orphaned file report**. |
...
- **Report:** `orphanedFiles` is `{adapter, path}` per filesystem holding the file; `missingFiles` is `{resource, adapter, path}` per uploadable row whose stored path has no object. **A missing file is only reported, never deletable**: it has no object and its row still references it, so a delete request for it is rejected as `not_found`.

...
| `silverback:api-components:scan-orphaned` (alias `clean-orphaned`) | The scheduled-run path (#348): scans synchronously through `ScanOrphanedResourcesHandler::scanAndCompare()`, stores the report, prints counts per kind (IRIs with `-v`), emails the configured recipients when the result changed (`--no-notify` skips it), exits 0. |
...
- **The report lives in the database** (#348), one row of `OrphanedResourceReportRecord` (`_acb_orphaned_resource_report`, fixed id 1) updated in place, so every pod reads the same report and it survives deploys. Never back it with `cache.app`: the template's pool is per-pod filesystem. `save()` returns the report it replaced; that is the only "previous" there is, so no history is kept. Nothing is held on a shared service, so no `kernel.reset`.
...
- **Notifications (#348):** `OrphanedResourceNotifier` emails `orphaned_resources.notify.recipients` when `OrphanedResourceReportChange::hasChanged()`: the IRI sets per kind differ from the replaced report (order ignored). A first report counts as a change only if it has orphans. The email carries counts per kind, IRIs new since the last report, a count of those resolved, and a link to `notify.admin_page_path` (default `/_cwa/orphaned`) on `user.email_links.default_origin`, through `RefererUrlResolver::getDefaultOriginUrl()`, which ignores any request origin. No usable default origin sends the email without the link and logs a warning.
- **Only the command notifies.** The HTTP scan and the refresh after a bulk delete call `scan()`, never the notifier: the admin is already looking. They still replace the stored report, so the next scheduled run compares against whatever was stored last, including an admin's scan.

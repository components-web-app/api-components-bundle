Feature: Reporting and deleting orphaned files
  ...
  Scenario: A stored file no row references is reported on every uploadable filesystem that holds it, and the imagine cache is not scanned
    Given there is a stored file "orphan.png" older than the minimum age
    And there is a stored file "components/nested/orphan.txt" older than the minimum age
    And there is a stored file "cache/thumbnail/orphan.png" older than the minimum age
    ...
    And the JSON node "orphanedFiles" should list exactly the files "local:components/nested/orphan.txt, local:orphan.png, public_url_local:components/nested/orphan.txt, public_url_local:orphan.png"
  ...
  Scenario: A file a row references is not reported, including one only a draft references
    ...
    And there is a stored file "orphan.png" older than the minimum age
    When I send a "POST" request to "/_/orphaned_files/scan"
    And I send a "GET" request to "/_/orphaned_files"
    Then the response status code should be 200
    And the JSON node "orphanedFiles" should list exactly the files "local:orphan.png, public_url_local:orphan.png"
    And the JSON node "missingFiles" should have 0 elements
  ...
  Scenario: A user without the administrator role cannot delete orphaned files
    ...
    When I send a "POST" request to "/_/orphaned_files/delete" with body:
      ...
      """
    Then the response status code should be 403
    And the stored file "orphan.png" should exist
  ...
  Scenario: Deleting selected paths deletes only those, and the stored report no longer lists them
    Given there is a stored file "first.png" older than the minimum age
    And there is a stored file "second.png" older than the minimum age
    When I request the deletion of the orphaned files "first.png"
    Then the response status code should be 200
    And the JSON node "deleted" should list exactly the files "local:first.png, public_url_local:first.png"
    And the JSON node "rejected" should have 0 elements
    And the stored file "first.png" should not exist
    And the stored file "second.png" should exist
    When I send a "GET" request to "/_/orphaned_files"
    Then the JSON node "orphanedFiles" should list exactly the files "local:second.png, public_url_local:second.png"

  @loginAdmin
  Scenario: Deleting all orphaned files deletes everything a fresh scan reports and nothing else
    Given there is a stored file "first.png" older than the minimum age
    And there is a stored file "components/second.png" older than the minimum age
    ...
    When I send a "POST" request to "/_/orphaned_files/delete" with body:
      ...
      """
    Then the response status code should be 200
    And the JSON node "deleted" should list exactly the files "local:first.png, local:components/second.png, public_url_local:first.png, public_url_local:components/second.png"
    And the JSON node "rejected" should have 0 elements
    And the stored file "first.png" should not exist
    And the stored file "components/second.png" should not exist
  ...
  Scenario: A path that stopped being orphaned between the scan and the delete is rejected and kept
    Given there is a stored file "claimed.txt" older than the minimum age
    ...
    And the resource "claimant" has a file "claimed.txt"
    And I add "Accept" header equal to "application/ld+json"
    And I add "Content-Type" header equal to "application/ld+json"
    When I request the deletion of the orphaned files "claimed.txt"
    Then the response status code should be 200
    And the JSON node "deleted" should have 0 elements
    And the JSON node "rejected[0].path" should be equal to "claimed.txt"
    And the JSON node "rejected[0].reason" should be equal to "not_orphaned"
    And the stored file "claimed.txt" should exist
  ...
  Scenario: The scan command prints the counts, lists the files with -v, stores the report and deletes nothing
    ...
    And the console command output should contain "  local: orphan.png"
    And the console command output should contain "  public_url_local: orphan.png"

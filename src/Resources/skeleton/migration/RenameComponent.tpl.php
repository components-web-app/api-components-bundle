...
final class <?php echo $class_name; ?> extends AbstractMigration
{
    ...
    {
        $this->addSql("UPDATE <?php echo $component_table; ?> SET dtype = '<?php echo $new_dtype; ?>' WHERE dtype = '<?php echo $old_dtype; ?>'");
        $this->updateAllowedComponents('<?php echo $old_iri; ?>', '<?php echo $new_iri; ?>');
    }

    public function down(Schema $schema): void
    {
        $this->addSql("UPDATE <?php echo $component_table; ?> SET dtype = '<?php echo $old_dtype; ?>' WHERE dtype = '<?php echo $new_dtype; ?>'");

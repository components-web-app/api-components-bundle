...
### Fixed
- A fixture `redirect()` whose `name:` belongs to a route created in the same load throws even when its path already exists, instead of re-pointing that name at the existing route ([#350](https://github.com/components-web-app/api-components-bundle/issues/350))

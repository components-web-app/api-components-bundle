...
### Added
- Orphaned file report (admin): `POST /_/orphaned_files/scan`, `GET /_/orphaned_files` and `silverback:api-components:scan-orphaned-files` report stored files under the uploadable fields' filesystems and prefixes that no row references (older than `orphaned_files.minimum_age`, default one hour; imagine cache and `orphaned_files.excluded_paths` excluded), and rows whose file is missing; `POST /_/orphaned_files/delete` deletes selected paths or all, re-checked against a fresh scan. Upgrade step: the report is stored in a new table, `_acb_orphaned_file_report`; generate and run a migration ([#373](https://github.com/components-web-app/api-components-bundle/pull/373))

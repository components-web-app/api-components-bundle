...
### Fixed
- The orphan scan reports what is orphaned only through an orphaned parent: the positions in an orphaned group, the components only those positions hold, the groups those components own, to any depth, and cycles nothing live reaches. A resource still reached from a live page, layout, component or page data is not listed, and `POST /_/orphaned_resources/delete` accepts the newly listed IRIs

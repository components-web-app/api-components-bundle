...
### Added
- Fixture builder: `allow:` on page groups, `liveAt()` on pages and page data, `redirect()` and `afterRoutes()` ([#321](https://github.com/components-web-app/api-components-bundle/issues/321))

### Fixed
- `generate-fixtures` writes a scaffold that loads and gives back the same site: published state and drafts, uploaded files, component-owned and shared groups, relations, dates, inherited and non-public fields, meta descriptions, scheduled routes and redirects. It lists anything it cannot reproduce ([#321](https://github.com/components-web-app/api-components-bundle/issues/321))

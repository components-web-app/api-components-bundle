...
### Fixed
- An admin's `POST /_/orphaned_resources/scan`, or the refresh after `POST /_/orphaned_resources/delete`, no longer absorbs an orphan change: `scan-orphaned` compares with the orphans at the last alert, kept in the same table, instead of with the last stored report. A failed send is retried on the next run ([#355](PR_URL))
